...
namespace PS.HireRocks.Web.Controllers
{
    public class AccountController : BaseController
    {
        ...
        {
            return new RoleRepository().GetAccountTypes().Select(x => new SelectListItem { Text = x.AccountTypeName, Value = x.AccountTypeId });
        }
        ...
        public async Task<ActionResult> Register(RegisterViewModel model)
        {
            if (ModelState.IsValid && model.IsUserAgreed)
            {
                ...
                if (result.Succeeded)
                {   
                    var roleResult = await UserManager.AddToRoleAsync(user.Id, model.RoleId);
